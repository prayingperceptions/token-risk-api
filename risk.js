// Token risk aggregation from free public data sources.
// Sources: Dexscreener public API (pairs/liquidity/volume), GoPlus Security API
// free tier (contract flags, holder concentration, honeypot signals).
// No API keys required for either at time of build (2026-09-22). Each fetch has
// a hard timeout; partial failures degrade gracefully with explicit nulls.
...
function isAddress(q) {
  return /^0x[a-fA-F0-9]{40}$/.test(q) || /^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(q); // evm or solana-ish
}
...
async function fromDexscreener(query) {
  ...
  // (e.g. querying USDC returning the AERO/USDC pair as "AERO").
  ...
  // Highest-liquidity pair among correct-token candidates is the canonical read.
  candidates.sort((a, b) => (b.liquidity?.usd || 0) - (a.liquidity?.usd || 0));
  const p = candidates[0];
  const liquidityUsd = p.liquidity?.usd ?? null;
  const volume24h = p.volume?.h24 ?? null;
...
function scoreRisk(dex, gp) {
  // 0 = safest, 100 = worst. Transparent, documented weights.
  let score = 0;
  const reasons = [];
...
export async function checkToken(query, chainHint) {
